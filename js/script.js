...
async function displayAlbums() {
    ...
    for (let index = 0; index < array.length; index++) {
        ...
        if (e.href.includes("/songs") && !e.href.includes(".htaccess")) {
            ...
            <div class="play">
            <img width="35" id="next" src="img/playnow.svg" alt="" />
            </div>
...
async function main(){
    // Getting songs 
    await getSongs("songs/tenta") 
    playMusic(songs[0], true)

    // Display Album on page
    await displayAlbums()

    // attach eventlistner to play and pause songs
    ...
    // Add eventlistner for Previous song
    ...
    // Add eventlistner for next song
    previous.addEventListener("click", () => {
        currentSong.pause()
        console.log("Previous clicked")
        let index = songs.indexOf(currentSong.src.split("/").slice(-1)[0])
    ...
    // Add eventlistner for menu option(hamberger)
    document.querySelector(".hamburger").addEventListener("click", ()=>{
        document.querySelector(".left").style.left = "0"
    })

    // evenlistner for mobile
    ...
    // Add eventlistner for closing menu option
